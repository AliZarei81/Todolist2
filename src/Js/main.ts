...
const checkItem = (itemId: number): void => {
    ...
    const itemElement = document.getElementById(`item-${itemId}`) as HTMLDivElement;
  const text = itemElement.querySelector(`#text-${itemId}`) as HTMLElement;
  const btn = itemElement.querySelector(`#check-${itemId}`) as HTMLButtonElement;
  const priority = itemElement.querySelector(`#item-priority-${itemId}`) as HTMLElement;
  const toolTip = itemElement.querySelector(`#checked-${itemId}`) as HTMLElement;

  text.classList.add("checked");
  btn.disabled = true;
  btn.classList.add("disabled");
  priority.classList.add("priority-checked");
  toolTip.innerText = "Checked!";
};
...
const deleteItem = (itemId: number): void => {
    // your code for check btn here ---------------------------------------->
    const index = data.toDoArray.findIndex(item => item.id === itemId);
  if (index !== -1) {
    data.toDoArray.splice(index, 1);
  }
  renderToDoList();
};
...
const setEditEvents = (itemId: number): void => {
     ...
     const cancelBtn = document.querySelector("#edit-no") as HTMLButtonElement;
     const changePriorityBtn = document.querySelector("#edit-priority") as HTMLButtonElement;
     const confirmBtn = document.querySelector("#edit-yes") as HTMLButtonElement;
     const newText = document.querySelector(`#edit-input-${itemId}`) as HTMLInputElement;
     ...
     confirmBtn.addEventListener("click", () => {
       const task = data.toDoArray.find((task) => task.id === itemId);
